...
def find_reader():
    ...
    if EXPECTED_READER_SUBSTR:
        for r in rs:
            if EXPECTED_READER_SUBSTR in str(r):
                return r
...
def build_event_envelope(state: ConnState, payload: bytes) -> bytes:
    """
    SICCT-Event-Envelope, best effort.
    FU Terminal = 0000.
    Eigene Event-Sequenz pro Verbindung.
    """
...
def send_slot_removed_event(state: ConnState, fu_number: int):
    """
    Slotereignis – Karte entfernt '85'
    Daten: 2 Byte FU-Nummer
    """
    with state.lock:
...
def rapdu_eject_icc(apdu: bytes, state: ConnState) -> bytes:
    """
    EJECT deaktiviert die Karte, Session bleibt offen.
    Zusätzlich senden wir kurz danach:
    Slotereignis – Karte entfernt '85'
    """
    with state.lock:
        if state.session_sid is None:
    ...
    # Bestehendes Verhalten möglichst wenig ändern:
    return b"\x90\x00"
